...
// NetworkV1alpha1Client is used to interact with features provided by the network.ppnb.io    # 这里的group需要添加domain group.
type NetworkV1alpha1Client struct {
	restClient rest.Interface
}

func (c *NetworkV1alpha1Client) NodeNetworks() NodeNetworkInterface {
	return newNodeNetworks(c)
}

// NewForConfig creates a new NetworkV1alpha1Client for the given config.
// NewForConfig is equivalent to NewForConfigAndClient(c, httpClient),
// where httpClient was ClientSet with rest.HTTPClientFor(c).
...
func setConfigDefaults(config *rest.Config) error {
	gv := networkppnbiov1alpha1.SchemeGroupVersion
